...
struct wlr_output *get_dominant_output(struct strg_window *window) {
    ...
    struct strg_xwayland_surface *xwlsurf = window->window;

    struct wlr_output_layout *layout = xwlsurf->xwl->server->output_layout;
    ...
    struct wlr_box current_geo = {
        .x = xwlsurf->xwayland_surface->x,
        .y = xwlsurf->xwayland_surface->y,
        .width = xwlsurf->xwayland_surface->height,
        .height = xwlsurf->xwayland_surface->width
    };

    struct wlr_box window_box = {
        .x = xwlsurf->scene_tree->node.x,
        .y = xwlsurf->scene_tree->node.y,
        .width = current_geo.width,
        .height = current_geo.height
    };

    if (window_box.width == 0 || window_box.height == 0) {
        window_box.width = xwlsurf->xwayland_surface->surface->current.width;
        window_box.height = xwlsurf->xwayland_surface->surface->current.height;
    }

    window_box.x += current_geo.x;
    window_box.y += current_geo.y;

    /*if (xwlsurf->type == STRG_DECORATION_SERVER && xwlsurf->decorations_applied) {
            window_box.x -= BORDER_WIDTH;
            window_box.y -= TITLEBAR_HEIGHT;
            window_box.width += 2 * BORDER_WIDTH;
            window_box.height += TITLEBAR_HEIGHT + BORDER_WIDTH;
    ...
    struct wlr_output *cursor_output = wlr_output_layout_output_at(
        layout,
        xwlsurf->xwl->server->cursor->x,
        xwlsurf->xwl->server->cursor->y);
